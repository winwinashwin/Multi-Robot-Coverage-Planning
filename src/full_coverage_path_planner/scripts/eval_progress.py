...
class CoverageProgressEvaluatorNode:
    def __init__(self):
        ...
        self._static_map = rospy.wait_for_message(self._static_map_topic, OccupancyGrid)
        self._map_subscription = rospy.Subscriber(self._cov_map_topic, OccupancyGrid, self.map_callback)

        self._progress_publisher = rospy.Publisher('coverage_progress', Float64, queue_size=5)
    ...
    def map_callback(self, coverage_map: OccupancyGrid):
        ...
        counts = dict(zip(*np.unique(coverage_grid[static_grid != -1], return_counts=True)))
