...
output_file_path = (
    rospkg.RosPack().get_path("follow_waypoints") + "/saved_path/pose.csv"
)
waypoints = []
...
class GetPath(State):
    ...
    def execute(self, userdata):
        ...
        def wait_for_path_ready():
            ...
            with open(output_file_path, "w") as file:
                for current_pose in waypoints:
                    file.write(
                        str(current_pose.pose.pose.position.x)
                        + ","
        ...
        while not self.path_ready and not self.start_journey_bool:
            try:
                pose = rospy.wait_for_message(topic, PoseWithCovarianceStamped)
...
def main():
    ...
    with sm:
        ...
        StateMachine.add(
            "PATH_COMPLETE", PathComplete(), transitions={"success": "GET_PATH"}
        )
