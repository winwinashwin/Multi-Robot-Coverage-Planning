...
class TestTrackingPID(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.trajectory_finished_sub = rospy.Subscriber(
            "trajectory_finished",
            Bool,
            self.trajectory_finished_callback,
            queue_size=1,
        )
    ...
    def test_tracking_pid(self):
        ...
        self.assertNotEqual(
            p1_msg.pose.pose.position.x,
            p2_msg.pose.pose.position.x,
            "Trajectory point has not moved",
        )
        ...
        self.assertTrue(
            self.trajectory_finished, "Trajectory not finished in 30 seconds"
        )
